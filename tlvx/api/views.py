...
def change_keyboard(word):
    alphabet = {'q':u'й','w':u'ц','e':u'у','r':u'к','t':u'е','y':u'н','u':u'г','i':u'ш','o':u'щ','p':u'з','[':u'х',']':u'ъ','a':u'ф','s':u'ы','d':u'в','f':u'а','g':u'п','h':u'р','j':u'о','k':u'л','l':u'д',';':u'ж',':':u'ж','\'':u'э','"':u'э','z':u'я','x':u'ч','c':u'с','v':u'м','b':u'и','n':u'т','m':u'ь',',':u'б','<':u'б','.':u'ю','>':u'ю'}
    return ('').join(map(lambda i:alphabet.get(i.lower(),i),word))
...
class ProjectApiView(APIView):
    ...
    def handle_exception(self, exc):
        if isinstance(exc, InvalidRequestException):
            return project_api_response.Response(self.request_form_errors, 
...
class CaptchaRoot(ApiRoot):
    ...
    def del_old(self):
        self.clone_model.exclude(date__day=timezone.now().day, date__month=timezone.now().month, date__year=timezone.now().year).delete()
    def get_object(self):
        self.del_old()
        images = list(
    ...
    def get_data(self, orig_obj,key, request):
        new_obj = self.clone_model.create(key=key,img=File(open(MEDIA_ROOT+'/'+orig_obj.img.name)), right=orig_obj.right)
        root_url = reverse('client-index', request=request)
        return dict(
