...
class BuildingAdmin(admin.ModelAdmin):
    fieldsets = [
        (None,                       {'fields': ['street', 'num', 'active', 'plan', 'name', 'co']}),
        (u'Альтернативный адрес',   {'fields': ['street_alt', 'num_alt'], 'classes': ['collapse']}),
        (u'2gis',                   {'fields': ['lat', 'lng'], 'classes': ['collapse']}),
        (u'Дополнительно',             {'fields': ['city', 'square','date', 'date_in'], 'classes': ['collapse']}),
    ]
    list_display = ('get_address','get_address_alt','get_pp', 'square', 'co', 'active','plan')     
...
class COAdmin(admin.ModelAdmin):
    list_display = ('id','name', 'address','schedule','contacts', 'get_builds_sum','marker','in_map')     
...
class NoteAdmin(admin.ModelAdmin):
    fieldsets = [
        ...
        (u'Дополнительно', {'fields': ['num', 'date', 'ntype','addresses'], 'classes': ['collapse']}),
    ]
    list_display = ('date','num', 'header' ,'text')
    list_filter = ['date', 'ntype']
    list_editable = ('num',)
...
class RatesAdmin(admin.ModelAdmin):
    fieldsets = [
        ...
        (u'Дополнительно', {'fields': ['name','date', 'date_out'], 'classes': ['collapse']}),
    ]
...
class PaymentPointAdmin(admin.ModelAdmin):
    fieldsets = [
        (None, {'fields': ['address', 'name', 'payment']}),   
        (u'Дополнительно', {'fields': ['schedule', 'contacts'], 'classes': ['collapse']}),
    ]   
...
class PaymentAdmin(admin.ModelAdmin):
    fieldsets = [
        (None, {'fields': ['name','is_terminal', 'num','description', 'marker','hint_line']}),
    ]
    list_editable = ('name','marker','is_terminal','hint_line','num')
    list_filter = ['is_terminal']
    list_display = ('id','name', 'get_values','marker', 'is_terminal','hint_line','num')
...
class TreePageAdmin(StaticPageAdmin):
    ...
    list_display = ('id','name', 'display_name','have_content', 'parent','have_childs','attach')
...
class HelpPageAdmin(TreePageAdmin):
    fieldsets = [
        (None, {'fields': ['num','name','display_name', 'attach', 'content', ]}),        
        (u'Дополнительно', {'fields': ['parent'], 'classes': ['collapse']}),
    ]
    list_display = ('id','name', 'display_name','have_content', 'parent','num')
